...
Another open question is to what extent digital health is increasing the digital divide in society\cite{disparities-health}.
While Woebot enables more people to have access to digital health, this gap in terms of equity is widening.
The user chapter has already introduced that people without a smartphone cannot use the app and are therefore excluded from technological progress.
Of course, this also applies to all people without access to electricity or the internet.
The reason for this can be a low income or the lack of connections in rural areas.
But even if people have a smartphone with internet access, it must have a fingerprint sensor.
Otherwise, the security feature cannot be activated, and the chat is freely accessible to everyone.
This is obviously a problem.
A feature like locking the application with a password or PIN could be added to reduce this problem.
However, these are not the only obstacles that can prevent people from using technology.\\

Other barriers may be related to gender or skin color.
This does not seem to be the case with Woebot.
Woebot uses the name of the person using the chat and avoids explicit gender assignment.
Skin color has often been a problem with various technologies in the past.
For example, the British newspaper The Guardian reports an incident involving Flickr's auto-tagging system, in which photos of black people were tagged as ape or animal\cite{the-guardian-flickr}.
The local newspaper, the Evening Standard, reports on a similar incident involving skin color in technology\cite{the-evening-standard}.
It features a viral video of an automatic soap dispenser that does not react to darker-skinned persons.
However, Woebot does not contain any image recognition processes or other processes in which skin color would play a role.
In addition to that no abnormalities were found in the chat that could indicate discrimination against certain skin colors.
Woebot seems to partially address some mentioned issues.
The app is available for free, which combats financial problems of individuals.
